...
const Offers: React.FC = () => {
  ...
  async function submit() {
    ...
    try {
      setInfo('')
      setSubmitting(true)
      await apiPost('/availability/offers', {
  ...
  async function loadMine() {
    try {
      ...
      const data = await apiGet<{ items: any[]; total: number; page: number; page_size: number }>(`/availability/offers/mine?${params}`)
      setMine(data.items)
      setTotal(data.total)
    } catch (e: any) {
  ...
  async function searchOffers() {
    try {
      ...
      const data = await apiGet<{ items: any[]; total: number; page: number; page_size: number; has_more: boolean }>(
        `/availability/offers/search?${params}`
      )
  ...
  async function remove(id: number) {
    try {
      ...
      await apiDeleteVoid(`/availability/offers/${id}`)
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <LoadingButton
              onClick={() => { setSearchPage(1); searchOffers(); }}
      ...
      {searchLoading ? (
        ...
        <>
          <p style={{ marginBottom: 12, color: '#666' }}>
            {searchTotal} offre{searchTotal !== 1 ? 's' : ''} trouvée{searchTotal !== 1 ? 's' : ''}
          </p>
          <ul>
            {searchResults.map(o => (
              <li key={o.id} className="list-item">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <div>
                    <strong>{new Date(o.start_at).toLocaleString()} → {new Date(o.end_at).toLocaleString()}</strong>
                  </div>
                  {o.user_email && (
                    <div style={{ fontSize: '0.85em', color: '#666' }}>
                      Contact: {o.user_email}
                    </div>
                  )}
