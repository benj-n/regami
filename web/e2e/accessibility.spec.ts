...
// Helper to create AxeBuilder with common exclusions for third-party components
function createAxeBuilder(page: Page): AxeBuilder {
  return new AxeBuilder({ page })
    .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
    // Exclude third-party Leaflet map attribution (not controllable)
  ...
  test.describe('Color Contrast', () => {
    test('should have sufficient color contrast on all pages', async ({ page }) => {
      ...
      for (const pagePath of pages) {
        ...
        // Check specifically for color contrast issues (excluding third-party Leaflet)
